import React from 'react';
import NavigationButton from '../../../components/NavigationButton';

function CasesHeroSection() {
    return (
        <div className="max-w-4xl mx-auto text-center">
            <h1 className='text-4xl lg:text-5xl font-rajdhani font-bold text-white mb-6 leading-tight'>
                Cases de <br />
                <span className="text-green-accent">Sucesso</span>
            </h1>

            <p className='text-lg font-league-spartan text-white/80 max-w-2xl mx-auto'>
                Conheça os projetos que transformaram negócios e superaram expectativas.
                Cada solução foi desenvolvida sob medida para gerar resultados reais.
            </p>
        </div>
    );
}

export default CasesHeroSection; 