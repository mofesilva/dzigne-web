...
function NavigationCard({ caseData, direction }: { caseData: NavigationCase; direction: 'previous' | 'next' }) {
    const isPrev = direction === 'previous';

    return (
        <Link href={caseData.href} className="group block">
                ...
                <div className={`flex-1 flex flex-col justify-center p-6 md:p-8 ${isPrev ? 'items-start' : 'items-end text-right'}`}>
                    <div className={`flex items-center gap-2 mb-4`}>
                        ...
                        <span className="font-outfit text-xs text-eggshell/65 tracking-widest uppercase">
                            {isPrev ? 'Case Anterior' : 'Próximo Case'}
                        </span>
...
function CaseNavigationSection({ previousCase, nextCase }: CaseNavigationSectionProps) {
    ...
    const hasBoth = previousCase && nextCase;
            ...
            <div className="px-6 md:px-12 lg:px-20 2xl:px-32 pt-0 pb-20 md:pb-28">
                ...
                <div className="flex flex-col lg:flex-row justify-between gap-5">
                    {previousCase ? (
                        ...
                        >
                            <NavigationCard caseData={previousCase} direction="previous" />
                        </motion.div>
                    ) : <div />}
                    ...
                    {nextCase ? (
                        ...
                        >
                            <NavigationCard caseData={nextCase} direction="next" />
                        </motion.div>
                    ) : <div />}
                </div>
